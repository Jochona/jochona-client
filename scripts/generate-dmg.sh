...
pushd $BUILD_FOLDER
dsymutil app/Jochona.app/Contents/MacOS/Jochona -o Jochona-$VERSION.dsym || fail "dSYM creation failed!"
cp -R Jochona-$VERSION.dsym $INSTALLER_FOLDER || fail "dSYM copy failed!"
popd
...
macdeployqt $BUILD_FOLDER/app/Jochona.app $EXTRA_ARGS -qmldir=$SOURCE_ROOT/app/gui -appstore-compliant || fail "macdeployqt failed!"
...
echo Creating DMG
if [ "$SIGNING_IDENTITY" != "" ]; then
  create-dmg $BUILD_FOLDER/app/Jochona.app $INSTALLER_FOLDER --identity="$SIGNING_IDENTITY" --no-version-in-filename || fail "create-dmg failed!"
else
  create-dmg $BUILD_FOLDER/app/Jochona.app $INSTALLER_FOLDER --no-version-in-filename
  case $? in
    0) ;;
    2) ;;
...
if [ "$NOTARY_KEYCHAIN_PROFILE" != "" ]; then
  ...
  xcrun notarytool submit --keychain-profile "$NOTARY_KEYCHAIN_PROFILE" --wait $INSTALLER_FOLDER/Jochona.dmg || fail "Notary submission failed"

  echo Stapling notary ticket to DMG
  xcrun stapler staple -v $INSTALLER_FOLDER/Jochona.dmg || fail "Notary ticket stapling failed!"
fi

mv $INSTALLER_FOLDER/Jochona.dmg $INSTALLER_FOLDER/Jochona-$VERSION.dmg
